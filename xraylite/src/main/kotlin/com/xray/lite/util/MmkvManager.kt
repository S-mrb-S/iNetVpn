package com.xray.lite.util

import com.google.gson.Gson
import com.tencent.mmkv.MMKV
import com.xray.lite.dto.ServerAffiliationInfo
...
object MmkvManager {
    ...
    const val ID_SERVER_DEF_CONFIG = "SERVER_DEF_CONFIG"
    ...
    private val serverDefStorage by lazy { MMKV.mmkvWithID(ID_SERVER_DEF_CONFIG, MMKV.MULTI_PROCESS_MODE) }
    ...
    fun decodeServerDefConfig(guid: String): ServerConfig? {
        if (guid.isBlank()) {
            return null
        }
        val json = serverDefStorage?.decodeString(guid)
        if (json.isNullOrBlank()) {
            return null
        }
        return Gson().fromJson(json, ServerConfig::class.java)
    ...
    fun encodeServerDefConfig(guid: String, config: ServerConfig): String {
        val key = guid.ifBlank { Utils.getUuid() }
        serverDefStorage?.encode(key, Gson().toJson(config))
        val serverList = decodeServerList()
        if (!serverList.contains(key)) {
            serverList.add(0, key)
            mainStorage?.encode(KEY_ANG_CONFIGS, Gson().toJson(serverList))
